...
class MainApplication(ttk.Window):
    ...
    def setup_ui(self):
        ...
        ttk.Button(self, width=50, text="Pre-processing", command=self.run_pre_processing, bootstyle="info").pack(pady=5)
        ttk.Button(self, width=50, text="Create Heterogeneous Graph", command=self.run_network_creation, bootstyle="success").pack(pady=5)
        ttk.Button(self, width=50, text="Extract Embeddings (NLP)", command=self.run_embedding_extraction, bootstyle="success").pack(pady=5)
        ttk.Button(self, width=50, text="Train GCN", command=self.run_gcn_extraction, bootstyle="warning").pack(pady=5)
        ttk.Button(self, width=50, text="GHAC Pre-processing", command=self.run_dividir_json_por_autor, bootstyle="secondary").pack(pady=5)
        ttk.Button(self, width=50, text="GHAC Clustering", command=self.run_clustering_validation, bootstyle="secondary").pack(pady=5)
